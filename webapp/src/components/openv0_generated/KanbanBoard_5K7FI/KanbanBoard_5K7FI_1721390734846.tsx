...
export default function KanbanBoard() {
  ...
  const handleAddTask = (column) => {
    ...
    if (taskName) {
      setTasks((prevTasks) => ({
        ...prevTasks,
        [column]: [...prevTasks[column], taskName],
      }));
    }
  };

  return (
    <div className="flex flex-col space-x-4 p-4 bg-gray-100 dark:bg-black w-full">
            ...
            <Card className="mb-2 h-full">
              <CardHeader>
                <CardTitle className="flex justify-between">
                  <span>{column}</span>
                  <span className="font-bold">{tasks[column].length} Pending</span>
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                {tasks[column].map((task, taskIndex) => (
                  <div
                    ...
                    className="flex items-center justify-between p-2 bg-white dark:bg-gray-800 border border-gray-300 rounded-md"
                  >
                    <span>{task}</span>
              ...
              <CardFooter>
                <Button className="w-full" variant="outline" onClick={() => handleAddTask(column)}>
                  Add Task
                </Button>
              </CardFooter>
            </Card>
